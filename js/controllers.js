...
.controller('ProductlocationCtrl', ['$scope', '$sce', '$http' ,'$stateParams', '$filter', function($scope, $sce, $http, $stateParams, $filter) {
    $scope.loading = true;
    $http({method: 'JSONP', url: "http://www.liquidanatal.com/app/hubs/"+$stateParams.locationId+".json?callback=JSON_CALLBACK", responseType: "json"})
    .success(function(data, status) {
        $scope.loading = false;
        $scope.hubs = data.object;
    })
    ...
    $scope.searchFilter = function (stores) {
        var keyword = new RegExp($scope.nameFilter, 'i');
        return !$scope.nameFilter || keyword.test(hubs.Store.name);
    };
}])
